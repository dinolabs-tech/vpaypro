...
// Fetch customer data
$stmt = $conn->prepare('SELECT name, email, address, country, state, profile_picture, account_status FROM customers WHERE id = ?');
$stmt->bind_param('i', $_SESSION['user_id']);
$stmt->execute();
$stmt->bind_result($name, $email, $address, $country, $state, $profile_picture, $account_status);
$stmt->fetch();
$stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_profile'])) {
  ...
  // Validate country and state inputs
  $country = $_POST['country'];
  $state = $_POST['state'];

  // Update name, email, address, country, and state
  $stmt = $conn->prepare('UPDATE customers SET name = ?, email = ?, address = ?, country = ?, state = ? WHERE id = ?');
  $stmt->bind_param('sssssi', $name, $email, $address, $country, $state, $_SESSION['user_id']);
  $stmt->execute();
  $stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_status'])) {
  ...
  $stmt->bind_param('si', $new_status, $_SESSION['user_id']);
  $stmt->execute();
  $stmt->close();
              ...
              <div class="col-xl-4">
                ...
                <div class="card">
                  <div class="card-body profile-card pt-4 d-flex flex-column align-items-center">
                    ...
                    <img src="assets/img/<?php echo $profile_picture; ?>" alt="Profile" class="rounded-circle" style="width: 120px; height: 120px;">
                    <h2><?php echo $name; ?></h2>
                  </div>
                </div>
              ...
              <div class="col-xl-8">
                ...
                <div class="card">
                  <div class="card-body pt-3">
                    ...
                    <p>Your account is currently: <strong><?php echo ucfirst($account_status); ?></strong></p>
                    <form method="POST">
                      <?php if ($account_status == 'active'): ?>
                        <input type="hidden" name="account_status" value="disabled">
                        <button type="submit" name="update_status" class="btn btn-danger rounded">Disable Account</button>
                      <?php else: ?>
                        <input type="hidden" name="account_status" value="active">
                        <button type="submit" name="update_status" class="btn btn-success rounded">Enable Account</button>
                      <?php endif; ?>
                    </form>
                  </div>
                </div>
